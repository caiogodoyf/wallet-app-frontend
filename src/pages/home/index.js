...
const renderFinanceElements = (data) => {
  ...
  const totalSubtext = document.createTextNode("Total entries");
  ...
  const revenueSubtext = document.createTextNode("Revenues");
  ...
  const expensesSubtext = document.createTextNode("Expenses");
  ...
  const balanceSubtext = document.createTextNode("Balance");
...
const onLoadUserInfo = () => {
  ...
  // add user frist letter inside avatar
...
const onCreateFinanceRelease = async (target) => {
  try {
    ...
    if (result.error) {
      alert("Error adding new financial data.");
      return;
    }
    onCloseModal();
    onLoadFinanceData();
